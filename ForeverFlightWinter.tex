...
We performed some power calculations to determine whether or we could draw enough power from the sun to add significant flight time to our aircraft.  The suns energy needs to go through several steps to be converted into propeller thrust. First, the solar cells convert photon energy into an electric potential difference that causes current to flow from the cells terminals.  This process is very inefficient; typical solar cells convert only twenty percent of the suns energy into electrical power. The other factor in determining usable power is the quantity of solar cells on the plane.  The power output of our solar array is directly proportional to the combined area of our cells.  To calculate the net power from our solar array power output, we multiply the suns power per area, or intensity, by the net area of our cells, and then again by the cell efficiency.  
\[ P_{net} = (I_{sun})(A_{cells})(\eta_{cell}) \]
Using twelve 22\% efficient cells, we are able to draw 77 watts of solar power.  This is enough to power our planes motor at about half throttle.  This means that if we have direct sunlight overhead, we should be able to keep the plane in the air with solar power alone.
...
% Grady Bio
\textbf{Grady Watkins}
is a senior at UC Santa Cruz studying electrical engineering. In his free time he enjoys playing Ultimate Frisbee, rock climbing, surfing, and playing video games.
